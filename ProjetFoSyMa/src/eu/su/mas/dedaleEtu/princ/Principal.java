...
import eu.su.mas.dedaleEtu.mas.agents.dummies.MyExploAgent;
import eu.su.mas.dedaleEtu.mas.agents.yours.FSMAgent;
...
public class Principal {
	...
	private static HashMap<String,ContainerController> createContainers(Runtime rt) {
		...
		containerName=ConfigurationFile.LOCAL_CONTAINER5_NAME;
		pContainer = new ProfileImpl(ConfigurationFile.PLATFORM_HOSTNAME, ConfigurationFile.PLATFORM_PORT, ConfigurationFile.PLATFORM_ID);
		//pContainer = new ProfileImpl(null, 8888, null);
		pContainer.setParameter(Profile.CONTAINER_NAME,containerName);
		System.out.println("Launching container "+pContainer);
...
//		agentList.add(ag);	
		...
		//4) Give the class name of your agent to let the system instantiate it
		ag=createNewDedaleAgent(c, agentName, FSMAgent.class.getName(), entityParameters2);
		...
		//3) If you want to give specific parameters to your agent, add them here
		Object [] entityParameters4= {"My param"};//{"Collect","Gold"};
		
		//4) Give the class name of your agent to let the system instantiate it
		ag=createNewDedaleAgent(c, agentName, FSMAgent.class.getName(), entityParameters4);
		//ag=createNewDedaleAgent(c, agentName, ExploreSoloAgent.class.getName(), entityParameters2);
		agentList.add(ag);*/
		/*********
		 * AGENT Silo
		...
		//3) If you want to give specific parameters to your agent, add them here
		Object [] entityParameters22= {"My param"};//{"Collect","Gold"};
		
		//4) Give the class name of your agent to let the system instantiate it
		ag=createNewDedaleAgent(c, agentName, FSMAgent.class.getName(), entityParameters22);
		//ag=createNewDedaleAgent(c, agentName, ExploreSoloAgent.class.getName(), entityParameters2);
		agentList.add(ag);
