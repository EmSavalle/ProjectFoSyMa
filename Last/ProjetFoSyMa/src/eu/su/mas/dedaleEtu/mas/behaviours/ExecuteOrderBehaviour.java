...
public class ExecuteOrderBehaviour extends OneShotBehaviour {
	...
	public void action() {
		...
		else if(this.data.executingOrder && this.data.order_agentnumber>= this.data.myMap.getShortestPath(this.data.myAgent.getCurrentPosition(), this.data.destination).size()) {//FIXME
			this.data.arrivedAtOrderDestination();
		}
		if (myPosition!=null){
